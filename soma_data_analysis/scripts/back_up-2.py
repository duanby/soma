...
from data_analysis import SOMAManager
...
class identify_objects:
     ...
     def asign_label(self,waypoint,instance):
         ...
         i=0
         #for num in prob.max(1):python -c "import scipy; print scipy.version.full_version"
             #if num<0.5:
                #indexs[i]=11
         for index in indexs:
             if index<11:
         ...
         print prob[30],indexs[30],self.label[instance][30]
     ...
     def set_box(self,waypoint,instance,objects):
         ...
         for i in range(len(centroids)):
             ...
             if len(part_feature)<100:
                ...
                self.objects[instance][objects][i]=[maxi,mini,aver]
             soma=SOMAManager('map1','conf')
             soma.add(objects,aver,waypoint)
...
if __name__ == "__main__":
   ...
   #mean,cov=objects.single_object_over_time(unidecode(req[0]),req[1],label_type[4],0,draw_region=1)
   objects.bounding_box(unidecode(req[0]),req[1][0],label_type[7]) 
   #objects.actaul_clouds(unidecode(req[0]),req[1][0],label_type[7])
